...
from pytorch_models import gru
from pytorch_models import attention
...
class BayesianGRUClassifier(nn.Module):

    def __init__(self, embedding_matrix, max_features, num_aux_targets, input_size, hidden_size):
        ...
        self.classifier_aux = nn.Sequential(
            OrderedDict([
                ...
                ('h1', nn.Linear(self.hidden_size * 4, 72)),
                ('relu1', nn.ReLU()),
                ('out', nn.Linear(72, num_aux_targets)),
            ]))
    ...
    def forward(self, _input, _features, hidden=None, lengths=None):
        _input = _input
        lengths = 220
        embedded = self.embedding(_input)
        ...
        result = self.classifier(concatenated)
        result_aux = self.classifier_aux(concatenated)
        out = torch.cat([result, result_aux], 1)
